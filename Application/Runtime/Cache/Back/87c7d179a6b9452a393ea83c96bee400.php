            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form action="<?php echo U('set');?>" method="post" enctype="multipart/form-data" id="form-brand" class="form-horizontal">
                        <?php if(isset($data['admin_id'])): ?><input type="hidden" name="admin_id" value="<?php echo ($data['admin_id']); ?>" id="input-admin_id"><?php endif; ?>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="tab-general">
                                    ...
                                    <label class="col-sm-2 control-label" for="input-name">登录名</label>
                                    <div class="col-sm-10">
                                        <input type="text" name="name" value="<?php echo ($data['name']); ?>" placeholder="登录名" id="input-name" class="form-control" data-remoteurl="<?php echo U('ajax');?>"/>
                                        <?php if(isset($message['name'])): ?><label for="input-name" class="text-danger"><?php echo ($message['name']); ?></label><?php endif; ?>
                                    </div>
                                </div>
                                <?php if(!isset($data['admin_id'])): ?><div class="form-group">
                                    <label class="col-sm-2 control-label" for="input-password">密码</label>
                                    <div class="col-sm-10">
                                        <input type="text" name="password" value="<?php echo ($data['password']); ?>" placeholder="密码" id="input-password" class="form-control" data-remoteurl="<?php echo U('ajax');?>"/>
                                        <?php if(isset($message['password'])): ?><label for="input-password" class="text-danger"><?php echo ($message['password']); ?></label><?php endif; ?>
                                    </div>
                                </div><?php endif; ?>
                                <div class="form-group">
                                    <label class="col-sm-2 control-label" for="input-level">权限等级</label>
                                    <div class="col-sm-10">
                                        <select name="level" id="input-level" class="form-control">
                                            <option value="0">请选择权限级别</option>
                                            <option value="1">超级管理员</option>
                                            <option value="2">普通管理员</option>
                                        </select>
                                        <?php if(isset($message['level'])): ?><label for="input-level" class="text-danger"><?php echo ($message['level']); ?></label><?php endif; ?>
                                    </div>
