...
class GoodsController extends Controller{
    ...
    public function listAction(){
        ...
        $model = M('Goods');
        $condition = [];
        $sort['filed'] = I('get.filed','update_at');
    ...
    public function setAction(){
        ...
        if(IS_POST){
            ...
            if(I('post.sort_number') == ''){
            	$_POST['sort_number'] = 200;
            }
            $model = M('Goods');
            //模型自动验证及填充
            if($model->create()){
            ...
            //分配商品系列
            $this->assign('goods_unit',M('Unit')->order('sort_number')->select());
            $this->display();
        }
    }
    //批量操作
    ...
    public function uploadAction(){
        ...
        if($info){
            ...
            switch ($type){
                case 'goods':
                    $image->thumb(340,340,Image::IMAGE_THUMB_FILLED);
